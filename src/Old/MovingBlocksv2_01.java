 ...
 * 
 */
public class MovingBlocksv2_01 extends JPanel implements MouseListener {
    ...
    public MovingBlocksv2_01(int width, int height) {
    ...
    public static void main(String[] args) {
        ...
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                f.setContentPane(new MovingBlocksv2_01(SCREEN_WIDTH, SCREEN_HEIGHT));
                f.pack();
                f.setVisible(true);
            }
        });
				...
				if(!startedThisShip && !busy) {	// start with this ship
					...
					if(containerIsDone) {
						...
						if(!hasContainer && y > destYCrane) { 		//move crane to the right position
							...
							destYCrane = WATER_HEIGHT + (25 * CONTAINER_WIDTH);
						} else if(y < destYCrane) {		// Move container to the left
							moveVert(1);
						//} else if() {		// move rod downwards
