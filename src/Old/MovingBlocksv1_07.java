 ...
 * 
 */
public class MovingBlocksv1_07 extends JPanel implements MouseListener {

	/**
	 * 
	 */
	private static String TITLE = "Moving blocks v1.07";
	...
	private static int CONTAINER_FIELD_WIDTH = 4 * CONTAINER_LENGTH + (CONTAINER_LENGTH /2);
    ...
    public MovingBlocksv1_07(int width, int height) {
    ...
    public static void main(String[] args) {
        ...
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                f.setContentPane(new MovingBlocksv1_07(SCREEN_WIDTH, SCREEN_HEIGHT));
                f.pack();
                f.setVisible(true);
            }
        });
...
*/
    ...
    public static class Ship extends Block {
		...
		public Ship(int ind) {
        	...
        	destX	= CONTAINER_FIELD_OFFSET + CONTAINER_FIELD_WIDTH - width;
            MaxSpeedX = 1 + (CONTAINER_LENGTH/10) + random(CONTAINER_LENGTH / 2);
            blokjes = new Block[xsize*ysize];
            for(int i=0;i<ysize;i++) {
            	for(int j=0;j<xsize;j++) {
        ...
        public void move(Container container) {
			...
			if(x >= destX) {
				if(tempCounter < 50) {
					tempCounter++;
				} else if(xsize > 4 && x < CONTAINER_FIELD_OFFSET){
